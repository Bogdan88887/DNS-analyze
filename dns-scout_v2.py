		...
		if modeV[1] in mode_pattern:
			try:
				if modeV[1] == mode_pattern[0]:
					Popen(['tshark', '-i', INTERFACE_, '-Y', 'dns'])
				else:
					Popen(["tshark", "-i", INTERFACE_, "-f", "port 53", "-w", "dns"+clock_+".pcap"])
...
#Добавить завершение предыдущих процессов, проверить пересылку пакетов...
